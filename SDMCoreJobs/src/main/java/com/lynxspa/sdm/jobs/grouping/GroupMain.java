package com.lynxspa.sdm.jobs.grouping;
...
import com.lynxspa.sdm.dictionary.flow.StaticDataWorkflow;
import com.lynxspa.sdm.dictionary.flow.state.StaticStatesSTATICMESSAGEFlow;
import com.lynxspa.sdm.entities.application.flow.Flow;
import com.lynxspa.sdm.entities.application.flow.State;
import com.lynxspa.sdm.entities.application.flow.StateId;
...
public class GroupMain {
	...
	public static void createConcatField() {
		...
		try {
			...
			// for(SecurityAsset
			// sa:(List<SecurityAsset>)statefullSession.createQuery("select sa from SecurityAsset sa where sa.concatField is null order by sa.id asc").list()){
			for (SecurityAsset sa : (List<SecurityAsset>) statefullSession
					.createQuery(
							"select sa from SecurityAsset sa where sa.completed is true order by sa.id asc")
					.list()) {
				...
				if (sa.getConcatField() == null
						|| sa.getConcatField().compareToIgnoreCase("") == 0) {
					sa.setConcatField(sa.getIsin() + separador + sa.getName()
							+ separador + sa.getCusip() + separador
							+ sa.getSedol() + separador + sa.getCountry());
				}
				groupFields(sa);
	...
	public static void groupFields(SecurityAsset sa) {
		...
		try {
			...
			if (sa.getConcatField().contains(separador)) {
				// rq =
				// statefullSession.createQuery("select sa from SecurityAsset sa where sa.concatField is not null and sa.id<>:id order by sa.id asc");
				rq = statefullSession
						.createQuery("select sa from SecurityAsset sa where sa.completed is true and sa.operationStatus.state.id.code = :state order by sa.id asc");
				// rq.setLong("id", sa.getId());
				rq.setString("state",
						StaticStatesSTATICMESSAGEFlow.MTCH.getId());
				if (rq.list().size() > 0) {
					for (SecurityAsset saRow : (List<SecurityAsset>) rq.list()) {
						...
						porcentajeAgrupacionActual = getPercentageGrouping(sa,
								saRow);
						// Si encontramos apariencias de un 80%
						if (porcentajeAgrupacionActual >= porcentajeAgrupacion
								&& (porcentajeAgrupacionActual > sa
										.getPercentageGrouping())) {
							System.out.println("% mayor q 80");
							sa.setPercentageGrouping(porcentajeAgrupacionActual);
							// Si tiene groupId lo utilizamos
							System.out.println("saRow.getGroupId():"
									+ saRow.getGroupId());
							if (saRow.getGroupId() != 0) {
								sa.setGroupId(saRow.getGroupId());
								sa.setOperationStatus(getOperationStatus(
										StaticStatesSTATICMESSAGEFlow.MTCH,
										statefullSession));
								/*
								 * if(porcentajeAgrupacionActual>saRow.
								 * getPercentageGrouping()){
								 * saRow.setPercentageGrouping
								 * (porcentajeAgrupacionActual); }
								 */
							} else {
								// Sino tiene groupId se crea de los dos
								// registros(comparador y comparado)
								saRow.setPercentageGrouping(porcentajeAgrupacionActual);
								long groupId = getId();
								saRow.setGroupId(groupId);
								saRow.setOperationStatus(getOperationStatus(
										StaticStatesSTATICMESSAGEFlow.MTCH,
										statefullSession));
	...
	public static long getId() {
		...
		try {
			it = statefullSession
					.createQuery(
							"select sa.groupId from SecurityAsset sa order by groupId desc")
					.iterate();
			if (it.hasNext()) {
	...
	public static int getPercentageGrouping(SecurityAsset saNuevo,
			SecurityAsset saExistente) {
		// Buscamos apariencias
		float percentageGrouping = 0;
		try {
			String[] camposNuevo = saNuevo.getConcatField().split(separador);
			String[] camposExistente = saExistente.getConcatField().split(
					separador);
			float apariencias = 0;
			float totalCampos = 0;
			if (camposNuevo.length == camposExistente.length) {
				...
				for (int i = 0; i < totalCampos; i++) {
					if (camposNuevo[i] != null
							&& camposNuevo[i].compareToIgnoreCase("null") != 0
							&& camposNuevo[i].compareToIgnoreCase("N.A.") != 0
							&& camposExistente[i] != null
							&& camposExistente[i].compareToIgnoreCase("null") != 0
							&& camposExistente[i].compareToIgnoreCase("N.A.") != 0
				...
				percentageGrouping = Math
						.round((apariencias / totalCampos) * 100);
				System.out.println("percentageGrouping:" + percentageGrouping);
			}
			System.out.println("totalCampos:" + totalCampos + " apariencias:"
					+ apariencias);
